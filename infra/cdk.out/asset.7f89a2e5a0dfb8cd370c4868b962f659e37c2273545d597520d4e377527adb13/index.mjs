...
function addItem(room, title, description) {
  const item = {
    id: randomUUID(),
    title,
    description,
  ...
  room.itemOrder.push(item.id);
  if (!room.currentItemId) {
    room.currentItemId = item.id;
    room.phase = "voting";
    room.votes.clear();
    room.reveal = null;
  }
...
function startVoting(room, itemId) {
  if (!room.items.has(itemId)) return;
...
function reveal(room, requesterId) {
  ...
  if (!room.currentItemId) return false;
  const scale = getScale(room.scale);
...
function markDiscussed(room, note) {
  if (!room.currentItemId) return;
  const item = room.items.get(room.currentItemId);
  if (!item) return;
  item.discussed = true;
  if (note && note.trim()) item.note = note.trim();
  const nextId = room.itemOrder.find((id) => {
    const candidate = room.items.get(id);
    return candidate && !candidate.discussed;
  });
  if (nextId) {
    room.currentItemId = nextId;
    room.phase = "voting";
  } else {
...
function recordToRoom(rec) {
  ...
  for (const i of rec.items) items.set(i.id, i);
...
var handler = async (event) => {
  ...
  switch (body.type) {
    case "add_item": {
      ...
      if (!title) return { statusCode: 200, body: "" };
      addItem(room, title, body.description?.trim() || void 0);
      break;
    }
    case "start_voting":
      startVoting(room, body.itemId);
      break;
    case "cast_vote":
